<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(auth()->check() && auth()->user()->isSeller())
                        <x-nav-link :href="route('seller.store.edit')" :active="request()->routeIs('seller.store.edit')">
                            {{ __('Profil Toko') }}
                        </x-nav-link>
                        ...
                        <x-nav-link :href="route('seller.balance.index')"
                            :active="request()->routeIs('seller.balance.index')">
                            {{ __('Saldo') }}
                        </x-nav-link>
                    @endif
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                @guest
                    <a href="{{ route('login') }}" class="px-3 py-2 text-sm font-medium text-gray-500 hover:text-gray-700">
                        Login
                    </a>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            {{-- Responsive menu untuk member --}}
            @if(auth()->check() && auth()->user()->isMember())
                <x-responsive-nav-link :href="route('store.register.form')">
                    {{ __('Daftar Toko') }}
                </x-responsive-nav-link>
            @endif

            {{-- Responsive menu untuk seller --}}
            @if(auth()->check() && auth()->user()->isSeller())
                <x-responsive-nav-link :href="route('seller.store.edit')">
                    {{ __('Profil Toko') }}
                </x-responsive-nav-link>
                ...
                <x-responsive-nav-link :href="route('seller.balance.index')">
                    {{ __('Saldo') }}
                </x-responsive-nav-link>
            @endif
        </div>
